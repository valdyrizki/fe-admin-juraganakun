...
untuk tips iklan, perawatan dan jika ada kendala, bisa hubungi tim support kami`
    ...
    const onSubmitComment = (e) => { //submit comment
        e.preventDefault()
                                ...
                                <div className="invoice p-3 mb-3">
                                    <div className="row">
                                        ...
                                        <h4>
                                            ...
                                            <small className="float-right">Date: {getFormatDate(current)}</small>
                                        </h4>
                                        </div>
                                    </div>
                                    <div className="row invoice-info">
                                        ...
                                        From
                                        <address>
                                            <strong>Juragan Akun</strong><br/>
                                            Phone: +6283818213645<br/>
                                        ...
                                        <address>
                                            ...
                                            Phone: {transaction.phone_number}<br/>
                            ...
                            <div className="row">
                                                ...
                                                {comments.map((comment)=>(
                                                    <div className="post" key={comment.id}>
                                                        <div className="user-block">
                                                            ...
                                                            <span className="description">Shared publicly - {comment.created_at}</span>
                                                        </div>
                                                        <p>
                                                            {comment.comment}
                                                        </p>
                                                        ...
                                                        <div className='row' >
                                                            {comment.comment_files.map((files)=>(
                                                                <p className='btn btn-dark btn-xs mr-1' key={files.id} onClick={(e) => downloadFileComment(files)}><i className="fas fa-link mr-1"></i>{files.filename}</p>
                                                            ))}
                                                        </div>
                                                        <span>
                                                            <p className="btn btn-danger btn-xs" onClick={()=>deleteComment(comment)}><i className="fas fa-trash mr-1"></i> Delete Comment</p>
                                                        </span>
                                                    </div>
                                                ))}

                                                <div className="post">
                                                    <form onSubmit={onSubmitComment} encType="multipart/form-data">
                                                        <div className="form-group col-12">
                                                            <b>Comment</b>
                                                            <textarea className='form-control form-control-sm' value={comment.comment} id="comment" name="comment" onChange={(e) => setComment({...comment,[e.target.name] : e.target.value})}  />
                                                        </div>
                                                        ...
                                                        <div className='row mt-4'>
                                                            {Array.from(files).map((file,index)=>(
                                                                <div className="col-12" key={index}>
                                                                    <p to="/#" className="link-black text-sm"><i className="fas fa-file-archive mr-1"></i> {file.name}</p>
                                                                </div>
                                                            ))}
                                                        </div>

                                                        <button type="submit" className="btn btn-sm btn-primary col-12">Submit Comment</button>
                                                    </form>
                                                </div>
                                            </>}
                                        </div>
                                ...
                                <div className="col-12 col-md-12 col-lg-4 order-1 order-md-2">
                                    ...
                                    <div className="text-muted">
                                        <p className="text-sm">Client Name
                                    ...
                                    <ul className="list-unstyled">
                                        {productFiles.map((product_file)=>(
                                            <li key={product_file.id}>
                                                <button className="btn btn-primary btn-xs" onClick={() => downloadFileProduct(product_file)}><i className="far fa-fw fa-file-archive"></i> {product_file.filename}</button>
                                            </li>
                                        ))}
                                    </ul>
                                    </div>
